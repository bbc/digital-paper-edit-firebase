...
const TranscriptTabContent = (props) => {
  const { transcriptId, title,
    media, labels, speakers, annotatedParagraphs,
  ...
  console.log(annotatedParagraphs, speakers);
  ...
  // useEffect(() => {
  //   const getGroupedParagraphs = async () => {
  //     const transcript = await collections.getTranscriptWithDecompression(projectId, transcriptId);
  //     setGroupedParagraphs(transcript.grouped);
  //   };
  //   if (collections) {
  //     setLabels(collections.getProjectLabels(projectId));
  //     setAnnotations(collections.getAnnotations(projectId, transcriptId));
  //     getGroupedParagraphs();
  //   }

  //   return () => {};
  // }, [ labels, collections, projectId, transcriptId ]);
  ...
  // useEffect(() => {
  //   const getUrl = async () => {
  //     const dlUrl = await collections.getTranscriptMediaUrl(projectId, transcriptId);
  //     setUrl(dlUrl);
  //   };
  ...
  //     getUrl();
  //   }
  // }, [ projectId, transcriptId, url, collections ]);
  ...
  const handleCreateAnnotation = (e) => {
    ...
    if (selection) {
      ...
      props.handleCreateAnnotation(transcriptId, newAnnotation);
    } else {
      alert('Select some text in the transcript to highlight ');
    }
  };
  const handleDeleteAnnotation = (annotationId) => {
    props.handleDeleteAnnotation(annotationId, transcriptId);
    const tempAnnotations = [ ...annotations ].splice(annotationId, 1);
    setAnnotations(tempAnnotations);
  };

  const handleEditAnnotation = (annotationId) => {
    const updateAnnotation = annotations.find((annotation) =>
    ...
    if (note) {
      props.handleEditAnnotation(annotationId, transcriptId, updateAnnotation);
    ...
    <>
      ...
      <Card>
        ...
        <Card.Header>
          <TranscriptMenu
            labels={ labels }
            updateLabelSelection={ props.handleLabelCreation }
            handleCreateAnnotation={ handleCreateAnnotation }
            onLabelCreate={ props.handleCreateLabel }
            onLabelUpdate={ props.handleUpdateLabel }
            onLabelDelete={ props.handleDeleteLabel }
          />
        </Card.Header>
        <SearchBar
